...
function storeRawInputs()
{
		...
		for(var i=1;i<=nbr;i++)
		{
			...
			text=getDataStructureRaw("RawList"+code,code);
			if(i!=nbr)
			{text+=","}				
...
function storeCleanInputs()
{
		...
		for(var i=1;i<=nbr;i++)
		{
			...
			text=getDataStructureClean("CleanList"+code,code);
			if(i!=nbr)
			{text+=","}				
...
function storeMappingChoice() // fonction qui enregistre le choix de l'utilisateur au départ s'il vaut des mapping de process ou non (utile pour l'affiochage et la gestion des "backs"
{
